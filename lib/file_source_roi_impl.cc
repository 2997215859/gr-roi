...
namespace gr {
  namespace roi {
...

    {
      ...
      file_source_roi_impl::~file_source_roi_impl() {
          if (fp) {
              fclose((FILE*)fp);
          }
          if (new_fp) {
       ...
       */
      void file_source_roi_impl::close() {
       ...
       * @return
       */
      bool file_source_roi_impl::seek(long seek_point, int whence) {
          return fseek((FILE*)fp, seek_point*itemsize, whence) == 0;
      }
      /**
       * 安装新的文件指针
       */
