...
//    taps.push_back(tap({2,3,4,4}));
    ...
    std::cout << "bitNum: " << bitNum << " whichSeq: " << whichSeq << std::endl;
    ...
    if (baseVal == 2) {
        for (int i=0;i<taps[whichSeq].size();i++) {
            weights[taps[whichSeq][i] - 1] = 1;
        }
...
namespace gr {
  namespace roi {
    ...
    satellite_file_sink::make(const char *filename, bool append, float threshold, int mseq_len, int mseq_cp_len, int mseq_index, int latency)
    {
      return gnuradio::get_initial_sptr
        (new satellite_file_sink_impl(filename, append, threshold, mseq_len, mseq_cp_len, mseq_index, latency));
    }

    /*
     * The private constructor
     */
    satellite_file_sink_impl::satellite_file_sink_impl(const char *filename, bool append, float threshold, int mseq_len, int mseq_cp_len, int mseq_index, int latency)
      : gr::block("satellite_file_sink",
        ...
        d_mseq_len(mseq_len),
        d_mseq_cp_len(mseq_cp_len),
        d_mseq_index(mseq_index),
        d_latency(latency),
        ...
        status_file(false)
    {
        ...
        sync_window = 128;  
        int power = round(log2(mseq_len+1));
        std::cout << "M sequence generated before power: " << power << " mseq_index: " << mseq_index <<  std::endl;
        vector<int> t_m_seq = generate_m_sequence(2, power, 0, mseq_index);
        
        std::cout << "m sequence generated size: " << t_m_seq.size() << std::endl;
        for (int i=0;i<t_m_seq.size();i++) {
            m_seq.push_back(gr_complex(0, t_m_seq[i]));
        }
        
        m_seq.insert(m_seq.begin(), m_seq.end() - mseq_cp_len, m_seq.end());
        for (int i=27;i<37;i++) {
     ...
     */
    satellite_file_sink_impl::~satellite_file_sink_impl()
    {
    }

//    void
//    satellite_file_sink_impl::forecast (int noutput_items, gr_vector_int &ninput_items_required)
//    {
...
//          std::cout << "status file: " << status_file << std::endl;
          ...
          if (avg >= d_threshold) { // 如果某段匹配到是m序列
             ...
             if (ferror(d_fp)) {
                 std::stringstream s;
                 s << "file sink write failed with error" << fileno(d_fp) << std::endl;
